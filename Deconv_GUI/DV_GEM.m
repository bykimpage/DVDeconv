function f = DV_GEM(g, h, curv, a, iter, Every_save_check, file_type)
...
for k = 1:iter
    ...
    if (Every_save_check == 1 && file_type == 1) % mat
        eval(['save DeconvGEM' num2str(k) ' f ']);
    elseif  (Every_save_check == 1 && file_type == 0) % tif
        for num=1:gz
            ...
            eval(['imwrite(mapped_image, ''DeconvGEM' num2str(k) '.tiff'', ''WriteMode'', ''append'');']);
        end
    end
